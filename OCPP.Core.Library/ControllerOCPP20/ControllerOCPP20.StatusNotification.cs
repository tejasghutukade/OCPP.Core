...
namespace OCPP.Core.Library
{
    public partial class ControllerOcpp20
    {
        public async Task<string?> HandleStatusNotification(OcppMessage msgIn, OcppMessage msgOut)
        {
            ...
            statusNotificationResponse.CustomData.VendorId = Library.ControllerOcpp20.VendorId;
            ...
            try
            {
                Logger.Verbose("Processing status notification...");
                ...
                Logger.Verbose("StatusNotification => Message deserialized");
                ...
                Logger.Information("StatusNotification => ChargePoint={0} / Connector={1} / newStatus={2}", ChargePointStatus?.Id, connectorId, newStatus.ToString());
                ...
                if (connectorId > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Logger.Verbose("StatusNotification => new OnlineConnectorStatus with values: ChargePoint={0} / Connector={1} / newStatus={2}", ChargePointStatus?.Id, connectorId, newStatus.ToString());
                        }
                        else
                        {
                            Logger.Error("StatusNotification => Error adding new OnlineConnectorStatus for ChargePoint={0} / Connector={1}", ChargePointStatus?.Id, connectorId);
                        }
                    }
                }
                else
                {
                    Logger.Warning("StatusNotification => Status for unexpected ConnectorId={1} on ChargePoint={0}", ChargePointStatus?.Id, connectorId);
                }

                msgOut.JsonPayload = JsonConvert.SerializeObject(statusNotificationResponse);
                Logger.Verbose("StatusNotification => Response serialized");
            }
            catch (Exception exp)
            {
                Logger.Error(exp, "StatusNotification => ChargePoint={0} / Exception: {1}", ChargePointStatus.Id, exp.Message);
                errorCode = ErrorCodes.InternalError;
            }
