...
namespace OCPP.Core.Server
{
    public partial class ControllerOCPP16
    {
        public string? HandleAuthorize(OCPPMessage msgIn, OCPPMessage msgOut)
        {
            ...
            try
            {
                ...
                try
                {
                    using (var dbContext = DbContext)
                    {
                        ...
                        Logger.LogInformation("Authorize => Status: {Status}", authorizeResponse.IdTagInfo.Status);
                    }
                }
                catch (Exception exp)
                {
                    Logger.LogError(exp, "Authorize => Exception reading charge tag ({IdTag}): {Message}", idTag, exp.Message);
                    authorizeResponse.IdTagInfo.Status = IdTagInfoStatus.Invalid;
                }
            ...
            {
                Logger.LogError(exp, "Authorize => Exception: {Message}", exp.Message);
