        ...
        public string? HandleStopTransaction(OCPPMessage msgIn, OCPPMessage msgOut)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(idTag))
                {
                    ...
                    Logger.LogInformation("StopTransaction => no charge tag => Status: {Status}", stopTransactionResponse.IdTagInfo.Status);
                }
                else
                {
                        ...
                        using (var dbContext = DbContext)
                        {
                            ...
                            if (transaction != null)
                            {
                                ...
                                if (valid)
                                {
                                    ...
                                    transaction.MeterStop =  (float?) ((double)stopTransactionRequest.MeterStop / 1000); // Meter value here is always Wh
